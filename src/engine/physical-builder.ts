...
export class PhysicalBuilder {
  ...
  build(plan: LogicalPlan): Operator {
    switch (plan.node) {
      ...
      case 'distinct':
        return new DistinctOp(this.build(plan.input), plan.labels);
  ...
  private outputLabelsOf(plan: LogicalPlan): string[] {
    ...
    if (plan.node === 'distinct' || plan.node === 'limit' || plan.node === 'sort') {
...
/**
 * Project 执行：结果以输出 label 为键。
 * ORDER BY 中未出现在结果列的表达式，由 Binder 作为隐藏投影项追加，
 * 在本算子内一并求值（此时底层列仍可用），排序后由 SortOp 删除。
 */
