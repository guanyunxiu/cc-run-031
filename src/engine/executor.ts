...
import type {
  LogicalPlan, LogicalJoin, BoundExpr, IndexUse, AggregateSpec,
} from './logical-plan';
...
/** 分组/连接哈希键（null 参与匹配：null = null 在 JOIN ON 中不为真，故单独分组永不匹配） */
...
export class SortOp extends Operator {
  ...
  open(frame: ExecFrame): void {
    ...
    // 隐藏排序键不外泄到结果行
    if (this.dropLabels.length > 0) {
      for (const row of this.rows) {
        for (const label of this.dropLabels) delete row[label];
      }
    }
    this.pos = 0;
  }
...
export class DistinctOp extends Operator {
  ...
  /**
   * @param labels 参与去重的列标签（可见结果列）；
   *               为 null 时回退到整行键（兼容无标签信息的用法）
   */
  constructor(private child: Operator, private labels: string[] | null = null) { super(); }
  open(frame: ExecFrame): void {
    this.seen.clear();
    this.child.open(frame);
  }
  next(frame: ExecFrame): EvalRow | null {
    for (;;) {
      ...
      const key = this.labels
        ? this.labels.map((k) => `${k}=${valueKey(row[k] ?? null)}`).join(';')
        : Object.keys(row)
