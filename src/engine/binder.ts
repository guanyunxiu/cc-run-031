...
export class Binder {
  ...
  bindSelect(stmt: SelectStmt): { plan: LogicalPlan; outputs: OutputColumn[] } {
    ...
    // 7. ORDER BY（先于 project 节点创建绑定：
    //    不在结果列中的排序表达式作为隐藏列追加到投影里，
    //    使其在投影前的行上求值；排序后再从行中删除这些隐藏键）
    const sortKeys: { expr: number; desc: boolean; nullsFirst: boolean }[] = [];
    const hiddenSortLabels: string[] = [];
    for (const o of stmt.orderBy) {
      let exprId: number;
      // 数字字面量：ORDER BY 序号（只对可见结果列计数）
      if (o.expr.kind === 'literal' && typeof o.expr.value === 'number' && Number.isInteger(o.expr.value)) {
        ...
        exprId = this.bindOrderExpr(o.expr, projectItems, outputs, scope, hiddenSortLabels);
      }
    ...
    if (stmt.distinct) {
      // 去重只看可见结果列，隐藏排序键不参与
      plan = { node: 'distinct', input: plan, labels: outputs.map((o) => o.label) };
    }
    if (sortKeys.length > 0) {
      plan = { node: 'sort', input: plan, keys: sortKeys, dropLabels: hiddenSortLabels };
    }
  ...
  private bindOrderExpr(
    ...
    outputs: OutputColumn[],
    scope: Scope,
    hiddenSortLabels: string[],
  ): number {
    // 纯列名时先匹配输出别名（SQLite 行为）；只在可见结果列中匹配
    if (e.kind === 'column' && !e.table) {
      const lower = e.name.toLowerCase();
      const aliasIdx = outputs.findIndex((o) => o.label.toLowerCase() === lower);
      if (aliasIdx >= 0) {
        return this.rebindOutputRef(projectItems[aliasIdx], outputs[aliasIdx], scope);
      }
    }
    // 排序键不在结果列中：绑定为针对投影前行的表达式，追加为隐藏投影列，
    // 排序键改为读取该隐藏列（排序算子输出前会删除这些隐藏键）。
    const bound = this.bindExpr(e);
    const hiddenLabel = genId('__orderby');
    projectItems.push({ expr: bound, label: hiddenLabel, type: 'ANY' });
    hiddenSortLabels.push(hiddenLabel);
    return this.pool.add({
      eval: (row) => row[hiddenLabel] ?? null,
      debug: `ref(${hiddenLabel})`,
      columnRefs: new Set([hiddenLabel]),
      constant: false,
    });
  }
