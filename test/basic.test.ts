...
describe('ORDER BY 非结果列排序', () => {
  async function seed() {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30,1),(2,10,3),(3,20,2)');
    return e;
  }

  it('130. ORDER BY 未在 SELECT 中的列', async () => {
    const e = await seed();
    expect(await column(e, 'SELECT id FROM t ORDER BY a')).toEqual([2, 3, 1]);
    expect(await column(e, 'SELECT id FROM t ORDER BY a DESC')).toEqual([1, 3, 2]);
  });

  it('131. ORDER BY 未在 SELECT 中的表达式', async () => {
    const e = await seed();
    expect(await column(e, 'SELECT id FROM t ORDER BY a + b')).toEqual([2, 3, 1]);
  });

  it('132. 排序键不外泄到结果列', async () => {
    const e = await seed();
    const r = await query(e, 'SELECT id FROM t ORDER BY a + b');
    expect(r.columns.map((c) => c.name)).toEqual(['id']);
    expect(r.rows).toEqual([{ id: 2 }, { id: 3 }, { id: 1 }]);
  });

  it('133. 别名与隐藏列混合排序键', async () => {
    const e = await seed();
    expect(await column(e, 'SELECT a AS x FROM t ORDER BY b DESC, x', 'x')).toEqual([10, 20, 30]);
    expect(await column(e, 'SELECT a AS x FROM t ORDER BY x DESC, b', 'x')).toEqual([30, 20, 10]);
  });

  it('134. 隐藏排序键配合 LIMIT', async () => {
    const e = await seed();
    expect(await column(e, 'SELECT id FROM t ORDER BY a LIMIT 2')).toEqual([2, 3]);
  });

  it('135. DISTINCT 只按可见列去重', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,30),(4,20)');
    // v 不在结果列：去重只看 id，再按代表行的 v 排序
    expect(await column(e, 'SELECT DISTINCT id FROM t ORDER BY v')).toEqual([2, 4, 1, 3]);
    // 可见列 DISTINCT 行为不变
    expect(await column(e, 'SELECT DISTINCT v FROM t ORDER BY v')).toEqual([10, 20, 30]);
  });

  it('136. ORDER BY 未在 SELECT 中的聚合', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (g TEXT, v INTEGER)');
    await e.execute("INSERT INTO t VALUES ('a',1),('b',1),('b',2),('c',1),('c',2),('c',3)");
    expect(await column(e, 'SELECT g FROM t GROUP BY g ORDER BY COUNT(*) DESC, g', 'g'))
      .toEqual(['c', 'b', 'a']);
  });
});
