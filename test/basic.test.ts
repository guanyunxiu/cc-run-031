...
describe('SELECT 基础 / WHERE / 别名', () => {
  ...
  it('130. ORDER BY 未出现在结果列中的列', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,20)');
    expect(await column(e, 'SELECT id FROM t ORDER BY v')).toEqual([2, 3, 1]);
    expect(await column(e, 'SELECT id FROM t ORDER BY v DESC')).toEqual([1, 3, 2]);
  });

  it('131. ORDER BY 未出现在结果列中的表达式', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, a INTEGER, b INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,5,1),(2,2,2),(3,1,5)');
    // a+b：4(2) < 6(1) = 6(3)，并列保持插入序
    expect(await column(e, 'SELECT id FROM t ORDER BY a + b')).toEqual([2, 1, 3]);
  });

  it('132. ORDER BY 隐藏排序键不出现在结果列中', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,20)');
    const r = await query(e, 'SELECT id FROM t ORDER BY v');
    expect(r.columns.map((c) => c.name)).toEqual(['id']);
    expect(r.rows).toEqual([{ id: 2 }, { id: 3 }, { id: 1 }]);
  });

  it('133. ORDER BY 非结果列 + LIMIT 取前 N', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,20)');
    expect(await column(e, 'SELECT id FROM t ORDER BY v LIMIT 2')).toEqual([2, 3]);
  });

  it('134. DISTINCT 只按结果列去重，ORDER BY 非结果列仍生效', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,20),(4,20)');
    // v=20 两行去重后保留首行（id=3），再按 id 排序
    expect(await column(e, 'SELECT DISTINCT v FROM t ORDER BY id')).toEqual([30, 10, 20]);
  });

  it('135. GROUP BY 后 ORDER BY 未出现在结果列中的聚合', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, g TEXT, v INTEGER)');
    await e.execute("INSERT INTO t VALUES (1,'a',10),(2,'b',30),(3,'a',20),(4,'b',5)");
    expect(await column(e, 'SELECT g FROM t GROUP BY g ORDER BY SUM(v) DESC')).toEqual(['b', 'a']);
  });

  it('136. 子查询源上 ORDER BY 非结果列', async () => {
    const e = await newEngine();
    await e.execute('CREATE TABLE t (id INTEGER PRIMARY KEY, v INTEGER)');
    await e.execute('INSERT INTO t VALUES (1,30),(2,10),(3,20)');
    expect(await column(e, 'SELECT id FROM (SELECT id, v FROM t) s ORDER BY s.v')).toEqual([2, 3, 1]);
  });
